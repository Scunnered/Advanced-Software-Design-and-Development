...
public class TimedRepeatAppointment extends TimedAppointment implements RepeatAppointment {
     ...
     */
    public TimedRepeatAppointment(String description, java.util.Date startDate, java.util.Date endDate, Time startTime, Time endTime, RepeatType type) {
    ...
    public java.util.Date getStartDate() {
        ...
        calendar.set(Calendar.YEAR, this.getDate().getYear());
        calendar.set(Calendar.MONTH, this.getDate().getMonth());
        calendar.set(Calendar.DAY_OF_MONTH, this.getDate().getDay());
    ...
    public boolean occursOn(Date date) {
        ...
        if (date.compareTo(Date.toGenericDate(startDate)) < 0)    //date is before start date of appointment
            return false;                   //impossible

        if (date.compareTo(Date.toGenericDate(endDate)) > 0)      //date is after end date of appointment
            return false;                   //impossible
        ...
        switch (this.repeatType) {
            ...
            case WEEKLY:
                Calendar startCalendar = Calendar.getInstance();                  //create Java Calendar for start date
...
//if there is no overlap, they won't clash even if occurring on the same date    
        ...
        Collection<java.util.Date> dates = this.getDates(); //get all dates this appointment occurs on
        for (java.util.Date date : dates) {
            if (a.occursOn(Date.toGenericDate(date)))   //the other one occurs on a date of this appointment
                return true;        //clash
        }
    ...
    public Collection<java.util.Date> getDates() {
        //call handy method provided in RepeatAppointment interface to do the job
